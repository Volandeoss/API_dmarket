...
def get_offer_from_market():
    market_response = req.get(
        rootApiUrl + "/exchange/v1/market/items?gameId=a8db&limit=1&currency=USD"
    )
...
def get_dmarket_lowest_offer(name: str, Only_Dmarket: bool):
    ...
    while True:
        ...
        params = {"Title": name, "limit": 100, "cursor": curs}
        resp = req.get(
            rootApiUrl + "/exchange/v1/offers-by-title", headers=headers, params=params
        )
        # print(resp.url)
        ...
        for offer in offers:
            if Only_Dmarket == True:
                if offer["inMarket"] is True:
                    print(
                        f"{y}. {int(offer['price']['USD'])/100} - {offer['inMarket']}"
                    )
...
def entry_on_enter(event):
    ...
    if main_func is not None:
        ...
        try:
            steam_lowest.config(
                text=f"{float(steamprice[1:])}-13% = {float(steamprice[1:]) - 13*float(steamprice[1:])/100}"
            )
        except:
